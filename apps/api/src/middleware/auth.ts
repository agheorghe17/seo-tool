 ...
 *
 * The verification path is chosen from the token's `alg` header:
 *  - HS*  → symmetric, needs `SUPABASE_JWT_SECRET` (legacy Supabase projects)
 *  - else → asymmetric via Supabase JWKS at `${SUPABASE_URL}/auth/v1/.well-known/jwks.json`
 */
...
async function ensureUserRow(userId: string, payload: JWTPayload): Promise<void> {
  ...
  try {
    ...
    /* retry provisioning on the next request if the DB was briefly unavailable */
  }
}
  ...
  try {
    ...
    if (alg.startsWith('HS')) {
      if (!env.SUPABASE_JWT_SECRET) {
        req.log.error('token is HS256 but SUPABASE_JWT_SECRET is not set');
        return reply.code(500).send({ error: 'auth not configured (set SUPABASE_JWT_SECRET)' });
      }
      ...
      return reply.code(500).send({ error: 'auth not configured (set SUPABASE_URL)' });
    }
